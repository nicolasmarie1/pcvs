...
@pytest.fixture()
def crit_comb():
    return {"arg": 10, "env": "message", "prog-env": "user_message", "prog-arg": "parameter"}
...
def test_combination_str(crit_desc, crit_comb):  # pylint: disable=redefined-outer-name
    ...
    crit_desc["arg"] = tested.Criterion(
        name="arg", numeric=True, description={"option": "-a ", "type": "argument", "subtitle": ""}
    )
    obj = tested.Combination(crit_desc, crit_comb, [])
    assert obj.translate_to_str() == "10_E=message_pa=parameter_pe=user_message"

    crit_desc["arg"] = tested.Criterion(
        name="arg", numeric=True, description={"option": "-a ", "type": "argument"}
    )

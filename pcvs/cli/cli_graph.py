...
@click.command(name="graph", short_help="Export graph from tests results.")
...
@click.pass_context
def cli_graph(ctx, bank_name, graph_types, path, show, extension, limit):  # pylint: disable=unused-argument
    if path is None:
        show = True
    else:
